...
<script type="text/javascript">
	...
	var deliveryCosts =	 <?php echo fetchDeliveryCosts(array('hostname'=>$theHostname, 'ordering'=>'ASC', 'orderBy'=>'minKm', 'useFormula'=>(($useDeliveryFormula == "false") ? false : true)), true); ?>;
	var region_ = '<?php echo $theRegion; ?>';
	var useFormula= <?php echo $useDeliveryFormula; ?>;
	...
	function updateDistanceResult(distance){
		...
		if(!useFormula) {
			...
			for(var i = 0; i < deliveryCosts.length; i++){
				...
				if(min <= distance && distance <= max) { //if distance is within this range
					if(distance > 0){
						$('#city-result').removeClass('hidden alert alert-error')
		...
		'. De bezorgkosten bedragen €'+formatEuro(deliveryCosts[i].price)+'.');
			...
			if(distance > delivery.absoluteMaxDistance){
				...
				$('#city-result').removeClass('hidden').html('We bezorgen helaas niet op deze afstand.');
			}
			else { //within reach
				$('#city-result')
